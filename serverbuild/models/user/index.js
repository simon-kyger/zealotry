...
var UserValidator = _interopRequireWildcard(_validate);
...
try {
  ...
  _assign__('User', _get__('mongoose').model('User'));
} catch (error) {
  const userSchema = new (_get__('mongoose').Schema)({
    username: {
      type: String,
      unique: true,
      validate: [{ validator: _get__('UserValidator').validateUsername, msg: 'Invalid username' }]
    },
    password: {
      type: String,
      validate: [{ validator: _get__('UserValidator').validatePassword, msg: 'Invalid password' }]
    },
    realm: {
      type: String,
      validate: [{ validator: _get__('UserValidator').validateRealm, msg: 'Invalid Realm' }]
    },
    characters: {
      type: [_get__('mongoose').Schema.ObjectId]
    }
  }, { timestamps: true });
  _assign__('User', _get__('mongoose').model('User', userSchema));
}
...
function _get_original__(variableName) {
  switch (variableName) {
    ...
    case 'UserValidator':
      return _filterWildcardImport__(UserValidator);
  }
...
let _typeOfOriginalExport = typeof User;
